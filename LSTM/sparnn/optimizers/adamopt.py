...
class AdamOpt(Optimizer):
    ...
    def __init__(self,
        ...
        super(AdamOpt, self).__init__(model, train_data_iterator, valid_data_iterator, test_data_iterator, hyper_param)
        self.learning_rate = numpy_floatX(hyper_param["learning_rate"])
        self.beta1 = numpy_floatX(hyper_param["beta1"]) # 0.1
        self.beta2 = numpy_floatX(hyper_param["beta2"]) # 0.001

    def set_name(self):
        self.name = "AdamOpt-" + self.id
    ...
    def get_update_func(self):
        ...
        m_t_list = [(b1 * g) + (1. - b1) * m for g, m in zip(self.grad, self.m_list)]
        v_t_list = [(b2 * TT.sqr(g)) + (1. - b2) * v for g, v in zip(self.grad, self.v_list)]
